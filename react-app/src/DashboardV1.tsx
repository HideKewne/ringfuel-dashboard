...
export default function DashboardV1() {
  ...
  // Toggle reactor (dark/light mode)
  ...
  const sources = [
    { name: 'Google Ads', label: 'Search / PPC Leads', position: 'float-top-left' },
    { name: 'Facebook Leads', label: 'Social Media', position: 'float-top-right' },
    { name: 'Cold Calls', label: 'Outbound', position: 'float-bottom-left' },
    { name: 'Referrals', label: 'Word of Mouth', position: 'float-bottom-right' },
  ];
  ...
  return (
    <>
          ...
          <div className="logo">
            <div className="logo-icon">
              <svg viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="20" cy="20" r="18" fill="url(#fuelGradient)" stroke="var(--primary-teal)" strokeWidth="2"/>
                <path d="M20 8C20 8 14 16 14 22C14 25.314 16.686 28 20 28C23.314 28 26 25.314 26 22C26 16 20 8 20 8Z" fill="url(#flameGradient)"/>
                ...
                <defs>
                  <linearGradient id="fuelGradient" x1="0" y1="0" x2="40" y2="40">
                    <stop offset="0%" stopColor="#0F1320"/>
                    <stop offset="100%" stopColor="#161C2D"/>
                  </linearGradient>
                  <linearGradient id="flameGradient" x1="20" y1="8" x2="20" y2="28">
      ...
      <main className="main-content">
        {/* Center Section - Reactor Core */}
        <section className="reactor-center" style={{ display: showSettings ? 'none' : '' }}>
          <div className="reactor-container" onClick={toggleReactor}>
            <div className="reactor-title">
              <h2 className="reactor-heading">THE REACTOR CORE</h2>
              <span className="reactor-mode-label">{isDark ? 'ON / FUELED' : 'OFF / STANDBY'}</span>
            </div>
            <div className="reactor-button">
              ...
              <svg viewBox="0 0 120 120" className="reactor-core-svg">
                <defs>
                  <radialGradient id="coreGlowOn" cx="50%" cy="50%" r="50%">
                    <stop offset="0%" stopColor="#FFFFFF"/>
                    <stop offset="20%" stopColor="#EAFFF8"/>
                    <stop offset="40%" stopColor="#29B6A8"/>
                    <stop offset="70%" stopColor="#1F9A8E"/>
                    <stop offset="100%" stopColor="transparent"/>
                  </radialGradient>
                  <radialGradient id="coreGlowOff" cx="50%" cy="50%" r="50%">
                ...
                <circle cx="60" cy="60" r="22" className="core-glow"/>
                <circle cx="60" cy="60" r="12" className="core-center"/>
        ...
        <div className="floating-stats" style={{ display: showSettings ? 'none' : '' }}>
          {sources.map((source, i) => (
            <div key={i} className={`source-card ${source.position}${sourceStates[i] ? ' online' : ''}`}>
          ...
          <section className="settings-page" style={{ display: 'flex' }}>
            <div className="settings-card">
              ...
              <div className="settings-tab-panel" style={{ display: activeTab === 'sources' ? '' : 'none' }}>
                <h2>Lead Sources</h2>
                <p className="tab-description">Manage your active lead sources. Changes sync with your dashboard.</p>
        ...
        <aside className="leaderboard-panel">
          <div className="panel-header">
            ...
            <svg className="trophy-icon" viewBox="0 0 24 24" fill="currentColor">
              <path d="M5 3h14c.55 0 1 .45 1 1v3c0 2.21-1.79 4-4 4h-.1c-.46 1.64-1.89 2.88-3.65 3.16V17h4v2H7v-2h4v-2.84c-1.76-.28-3.19-1.52-3.65-3.16H7c-2.21 0-4-1.79-4-4V4c0-.55.45-1 1-1zm0 2v2c0 1.1.9 2 2 2h.28c-.04-.16-.07-.33-.07-.5V5H5zm14 0h-2.21v3.5c0 .17-.03.34-.07.5H17c1.1 0 2-.9 2-2V5zM9 5v3.5c0 1.38 1.12 2.5 2.5 2.5s2.5-1.12 2.5-2.5V5H9z"/>
            </svg>
          </div>
          <div className="leaderboard-list">
            {[
              { rank: 1, name: 'Joe Vazquez', stats: '156 calls ~ $12,400', revenue: '$12,400', img: 3 },
              { rank: 2, name: 'Cole Bingham', stats: '134 calls ~ $10,200', revenue: '$10,200', img: 8 },
              { rank: 3, name: 'Anne Mitchell', stats: '121 calls ~ $9,800', revenue: '$9,800', img: 5 },
              { rank: 4, name: 'Marcus Chen', stats: '108 calls ~ $8,500', revenue: '$8,500', img: 12 },
              { rank: 5, name: 'Sarah Williams', stats: '95 calls ~ $7,200', revenue: '$7,200', img: 9 },
              { rank: 6, name: 'David Park', stats: '87 calls ~ $6,800', revenue: '$6,800', img: 15 },
              { rank: 7, name: 'Emma Rodriguez', stats: '79 calls ~ $5,900', revenue: '$5,900', img: 25 },
              { rank: 8, name: 'Michael Torres', stats: '72 calls ~ $5,400', revenue: '$5,400', img: 53 },
            ].map(agent => (
              <div key={agent.rank} className={`leaderboard-item${agent.rank === 1 ? ' rank-1' : ''}`}>
                ...
                <span className="agent-revenue">{agent.revenue}</span>
              </div>
            ))}
          </div>
        </aside>
